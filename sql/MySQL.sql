...
-- ========================
/* STRING */
CHAR(X) -- USED FOR FIXED LENGTH
VARCHAR(X) -- VARIABLE LENGTH, CAN BE INDEXED
...
/* FLOATING POINTS */
DECIMAL(PRESICION, SCALE) -- PRECISION - MAX.NO OF DIGITS, SCALE - MAX.NO OF DIGITS AFTER DECIMAL POINT
FLOAT -- 4byte
DOUBLE -- 4byte

/* BOOLEAN */
BOOL (OR) BOOLEAN -- (TRUE, FALSE) OR (0, 1)
...
/* BLOBS - STORE BINARY DATA LIKE PHOTOS, PDF */
...
-- EXTRACT
-- '$' - TO ACCESS THE CURRENT JSON DOCUMENT
-- '.' - TO ACCESS INDIVIDUAL PROPERTIES IN THE DOCUMENT
    SYNTAX: JSON_EXTRACT(<JSON_OBJECT>, '$.<KEY>') OR <JSON_OBJECT> -> '$.<KEY>'

-- TO GET THE JSON_ARRAY IN JSON OBJECT
    SYNTAX: <JSON_OBJECT> -> '$.<KEY>[INDEX]'

-- TO GET THE NESTED JSON KEY FROM THE JSON
    SYNTAX: <JSON_OBJECT> -> '$.<KEY>.<NESTED_KEY>'

-- USE '->>' TO EXTRACT STRING WITHOUT DOUBLE QUOTES
    SYNTAX: <JSON_OBJECT> ->> '$.<KEY>.<NESTED_KEY>'

-- SET - TO UPDATE / ADD VALUE
    JSON_SET(<JSON_OBJECT>,
          '$.KEY_1', VALUE,
          '$.KEY_2', VALUE,
          )

-- REMOVE
    JSON_REMOVE(<JSON_OBJECT>, '$.KEY_1', '$.KEY_2')
...
/* INTEGER */
SELECT ROUND(5.735, 1);
SELECT CEILING (5.735); -- RETURNS INTEGET THAT IS >= NUMBER
SELECT FLOOR(5.735); -- RETURNS INTEGET THAT IS <= NUMBER
SELECT ABS(-123); -- CALCULATE THE ABSOLUTE VALUE
SELECT RAND(); -- GET RANDOM NUMBER BETWEEN 0 AND 1
...
SELECT TRIM('  HELLO  '); -- LEADING OR TRAILING SPACE
SELECT LEFT('KINDERGARDEN', 4); -- GET CHARS FROM LEFT SIDE OF STRING
SELECT RIGHT('KINDERGARDEN', 6); -- GET CHARS FROM RIGHT SIDE OF STRING
SELECT SUBSTRING('KINDERGARDEN', 4, 2); -- GET CHARS FROM ANY WHERE FRROM THE STRING (WORD, INDEX, NUMBEROFCHAR FROM INDEX)
SELECT LOCATE('E', 'KINDERGARDEN'); -- RETURNS THE FIRST POSITION OF A CHAR OR SEQUENCE OF CHAR. NOT CASE SENSITIVE. IF NOT FOUND RESULT IS 0
SELECT REPLACE('KINDERGARTEN', 'GARTEN', 'GARDEN'); -- (WORD, SUBSTRING_TO_REPLACE, REPLACEMENT_STRING)
...
SELECT DAYNAME(NOW()) AS DAYNAME, MONTHNAME(NOW()) AS MONTHNAME ;
...
SELECT DATEDIFF('2024-10-08', '2024-10-2');

SELECT TIME_TO_SEC('1:00'); -- NUMBER OF SECOND FROM THE MIDNIGHT
SELECT TIME_TO_SEC('3:00') - TIME_TO_SEC('1:00');

/* IFNULL - SUBSITITUTE THE NULL VALUE WITH THE GIVEN VALUE
 * &
 * COALESCE - RETURNS THE FIRST NON-NULL VALUE */

SELECT IFNULL('FIRST', 'SECOND'), COALESCE ('FIRST', 'SECOND', 'THIRD');
SELECT IFNULL(NULL , 'SECOND'), COALESCE (NULL, NULL, 'THIRD');

/* IF
 * IF (EXPRESSION, RESULT_FOR_TRUE, RESULT_FOR_FALSE)
 * */
SELECT IF (YEAR(NOW()) = YEAR(NOW()), 'TRUE', 'FALSE') AS CODITIONS;
SELECT IF ( 2=1 , 'TRUE', 'FALSE') AS CODITIONS;
